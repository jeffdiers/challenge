...
const runServer = async () => {
  ...
  // serve the source map for the client.js
  try {
    const clientJsMap = fs.readFileSync('./dist/client.js.map', 'utf8');
    fastify.get('/client.js.map', async (_request, reply) => {
      reply.header('content-type', 'application/json').send(clientJsMap);
    });
  } catch (error) {
    console.warn(
      'client.js.map file not found. Skipping /client.js.map route.',
    );
  }

  // serve the service worker for msw to work in the browser
  const mswJs = fs.readFileSync('./dist/mockServiceWorker.js');
  fastify.get('/mockServiceWorker.js', async (_request, reply) => {
  ...
  fastify.get('/appWithSSRData', async (_request, reply) => {
    wipeCache();
    reply
      .header('content-type', 'text/html')
  ...
  fastify.get('/appWithoutSSRData', async (_request, reply) => {
    wipeCache();
    reply
      .header('content-type', 'text/html')
